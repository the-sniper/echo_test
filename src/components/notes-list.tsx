...
export function NotesList({ notes, sessionId, scenes, testers, onNoteUpdated, onNoteDeleted }: Props) {
  ...
  async function updateCategory(noteId: string, category: NoteCategory) { 
    try { 
      const res = await fetch(`/api/sessions/${sessionId}/notes/${noteId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category, auto_classified: false }) }); 
      if (res.ok) { onNoteUpdated(await res.json()); } 
    } catch {} 
    finally { setEditingCategoryId(null); }
  }
  async function updateScene(noteId: string, sceneId: string) { 
    try { 
      const res = await fetch(`/api/sessions/${sessionId}/notes/${noteId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scene_id: sceneId }) }); 
      if (res.ok) { onNoteUpdated(await res.json()); } 
    } catch {} 
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {filteredNotes.length === 0 && hasActiveFilters ? (
          ...
          <div key={note.id} className="p-4 rounded-lg border border-border bg-card/50 space-y-2 animate-fade-in">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {scenes && scenes.length > 0 ? (
                  editingSceneId === note.id ? (
                    ...
                    <span className="text-xs text-muted-foreground cursor-pointer hover:text-foreground" onClick={() => setEditingSceneId(note.id)}>{getSceneName(note)}</span>
                  )
                ) : (
                  "scene" in note && note.scene && <span className="text-xs text-muted-foreground">{note.scene.name}</span>
                )}
              </div>
              {((note.edited_transcript || note.raw_transcript) || onNoteDeleted) && (
                <DropdownMenu>
            ...
            {editingId === note.id ? (
              <div className="space-y-2">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button size="sm" onClick={() => saveEdit(note.id)}><Check className="w-4 h-4" />Save</Button>
                </div>
              </div>
            ) : (
              <p className="text-sm">{note.edited_transcript || note.raw_transcript || <span className="text-muted-foreground italic">[Transcription unavailable]</span>}</p>
            )}
