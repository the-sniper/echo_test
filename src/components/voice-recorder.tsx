...
export function VoiceRecorder({ sessionId, sceneId, testerId, sceneName, onNoteCreated }: Props) {
  ...
  async function processRecording() {
    try {
      ...
      setStep("Saving...");
      const noteRes = await fetch(`/api/sessions/${sessionId}/notes`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scene_id: sceneId, tester_id: testerId, raw_transcript: transcript, category: finalCategory, auto_classified: autoClassified }) });
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="pb-3"><CardTitle className="text-lg flex items-center justify-between"><span>Voice Note</span>{state !== "idle" && state !== "processing" && <span className="text-sm font-mono text-muted-foreground">{formatDuration(duration)}</span>}</CardTitle></CardHeader>
